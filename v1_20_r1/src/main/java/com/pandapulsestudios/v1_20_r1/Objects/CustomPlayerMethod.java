package com.pandapulsestudios.v1_20_r1.Objects;
...
public class CustomPlayerMethod implements CustomPlayerMethodNMS {
    ...
    public List<String> ReturnTabComplete(Player player, String[] args) throws InvocationTargetException, IllegalAccessException {
        ...
        if(method.isAnnotationPresent(PCFunctionHideTab.class) && canShowFunctionInTab){
            var classMethod = liveData.getOrDefault(method.getAnnotation(PCFunctionHideTab.class).value(), null);
            if(classMethod != null) canShowFunctionInTab = (boolean) classMethod.invoke(playerCommand, player.getUniqueId());
        }
    ...
    public boolean CanPlayerUseCommand(Player player) {
        ...
        if(method.isAnnotationPresent(PCRegion.class) && PluginAPI.IsPluginInstalled(javaPlugin, SoftDependPlugins.WorldGuard)){
    ...
    private List<String> ConvertPCTab(Player player, PCTab pcTab, int testLocation, int numberOfArguments, String currentInput) throws InvocationTargetException, IllegalAccessException {
        ...
        if(pcTab.type() == TabType.Information_From_Function){
            var method = liveData.getOrDefault(pcTab.data(), null);
            return method == null ? new ArrayList<String>() : (List<String>) method.invoke(playerCommand, currentInput);
